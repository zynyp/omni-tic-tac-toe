    ...
    return (
        <dialog ref={dialog} class="grid absolute top-0 left-0 place-items-center w-full h-full bg-[#00000040] opacity-0">
            <div ref={container} class="flex flex-col gap-6 items-center pt-6 pb-4 mx-4 my-12 w-full max-w-80 bg-white rounded-2xl">
                ...
                <button class="w-48 h-14 bg-blue-500 text-xl font-bold text-white rounded-full outline-none focus:ring-2 ring-blue-500 ring-offset-2 hover:brightness-90 transition duration-200" onclick={onClick} onpointerup={({ target }) => (target as HTMLButtonElement).blur()}>Restart Game</button>
            </div>
        </dialog>
    );
}
