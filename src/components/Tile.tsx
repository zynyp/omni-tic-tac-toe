...
export default function Tile(props: TileProps & JSX.HTMLAttributes<HTMLButtonElement>) {
    ...
    const { tile, tileCount, currentTurn, credits, allowPlacing, isSelected, isHidden = false, onclick } = props;
    ...
    return (
        isHidden
            ...
            : (
                <button ref={button} class={`aspect-square grid relative place-items-center w-full outline-none ${tile() === TileType.None ? "opacity-0" : ""} ${
                    allowPlacing() && tile() === TileType.None && credits() > 0 && tileCount()[placingTile()] < MAX_TILES[placingTile()]
                        ? canBeFocused()
                            ? "focus:opacity-50"
                            : "hover:opacity-50"
                        : ""
                }`} title={`${isSelected() ? "Selected" : ""} ${TileNames[tile()]} Tile`} onclick={onclick} onpointerenter={() => button.focus()} onpointerleave={() => button.blur()}>
                    ...
                    {isSelected() && <img class="absolute top-1/2 left-1/2 w-5 md:not-h-md:w-7 lg:not-h-md:w-9 not-md:h-md:w-7 not-md:h-lg:w-9 -translate-1/2" src="images/selected-shadow.svg" alt="Tile Selected" width={36} draggable="false" />} 
                </button>
            )
    );
}
